...
export const checkerboardMaterial = new Materials.GridMaterial('checkerboard', scene)
...
{
  crossHair.parent = thirdPersonCamera
  ...
  ;(globalThis as any)['setSunInclination'] = setSunInclination
  Object.assign(globalThis, { setCamera, firstPersonCamera, thirdPersonCamera, editorCamera, setSunInclination })
}

/// --- EXPORTS ---

export function setCamera(camera: BABYLON.Camera) {
...
export function reposition() {
  ...
  envHelper.rootMesh.position.set(
    Math.floor(scene.activeCamera!.position.x / PARCEL_SIZE) * PARCEL_SIZE,
    0,
    Math.floor(scene.activeCamera!.position.z / PARCEL_SIZE) * PARCEL_SIZE
  )
